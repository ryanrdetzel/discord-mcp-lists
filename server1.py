...
mcp = FastMCP(
    name="Discord Channel Lists",
    instructions="If no list name is provided use list name 'todo'. If an item is added or removed from a list return the number of items now in that list.",
)
...
def migrate_database():
    ...
    try:
        ...
        if "status" not in column_names:
            ...
            cursor.execute(
                "ALTER TABLE list_items ADD COLUMN status TEXT DEFAULT 'active'"
            )
        else:
...
def add_list_item(channel_id: str, list_name: str, item_name: str) -> str:
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO list_items (list_id, name) VALUES (?, ?)", (list_id, item_name)
        )
        conn.commit()
...
def get_lists(channel_id: str = None) -> str:
    ...
    try:
        if channel_id:
            cursor.execute(
                "SELECT id, name FROM lists WHERE channel_id = ?", (channel_id,)
            )
...
@mcp.tool()
def get_list_items(
    channel_id: str, list_name: str, show_completed: bool = False
) -> str:
